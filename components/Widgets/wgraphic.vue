<template>
    ...
    <card  type="chart">
        ...
        <template slot="header">
            ...
            <h5 class="card-category">{{ config.selectedVariable.variable_name }} - {{ config.variableFullName }}</h5>
...
<script>
    export default {
        ...
        mounted() {
            ...
            //this.topic = this.config.userId + '/' + this.config.selectedVariable.variable_id + '/' + this.config.variable;
            //this.$nuxt.$on(this.topic + "/sdata", this.procesReceivedData);
            this.getNow();
            this.updateColorClass();
        },
        beforeDestroy() {
            this.$nuxt.$off("data/" + this.topic, this.procesReceivedData);
        },
        methods: {
            updateColorClass() {
            ...
            getChartData() {
                ...
                const axiosHeaders = {
                    ...
                    params: { dId: this.config.selectedVariable.variable_id, variable: this.config.variable, chartTimeAgo: this.config.chartTimeAgo }
                }
                this.$axios.get("/get-small-charts-data", axiosHeaders)
                    .then(res => {
                        ...
                        data.forEach(element => {
                            var aux = []
                            aux.push(element.data_time + (new Date().getTimezoneOffset() * 60 * 1000 * -1));
                            aux.push(element.data_value);
